...
def upgrade() -> None:
    ...
    # Using try-except or check to be safe, but typically alembic migrations are prescriptive
    ...
    # 3. Create exam_violations table
    op.create_table(
        'exam_violations',
...
def downgrade() -> None:
    ...
    op.drop_column('course_exam_answers', 'is_correct')
    op.drop_column('exam_answers', 'is_correct')
    op.drop_column('course_exams', 'questions_per_attempt')
    op.drop_column('entrance_exams', 'questions_per_attempt')
